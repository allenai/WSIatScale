...
def main():
    ...
    if app_str == 'WSI at Scale':
        ...
        cluster_reps_to_present = st.sidebar.slider(f"Number of cluster replacements to present", 1, 100, 10)
        infer_senses_by_list(tokenizer, data_dir, method, n_reps, cluster_reps_to_present)

def infer_senses_by_list(tokenizer, data_dir, method, n_reps, cluster_reps_to_present):
    words = st.text_input("Word Senses to infer, comma seperated.", "bass, trout, salmon, drum")
    st.write('Ranked by aggregated log jaccard score between sense pairs.')
    words = words.replace(" ", "").split(',')

    flat_word_clusters_dict, flat_word_clusters_names, sense_counts = read_clusters_for_multiple_words(tokenizer, data_dir, words, method, n_reps, cluster_reps_to_present)

    sense_positions = create_sense_positions(words, sense_counts)

    jaccard_sims_heap = find_top_matches(sense_positions, flat_word_clusters_dict)

    print_top_matches(tokenizer, data_dir, method, n_reps, cluster_reps_to_present, jaccard_sims_heap, flat_word_clusters_names)

from utils.utils import timeit
@timeit
def print_top_matches(tokenizer, data_dir, method, n_reps, cluster_reps_to_present, jaccard_sims_heap, flat_word_clusters_names):
    for jaccard_score, prod in jaccard_sims_heap:
        ...
        for i in prod:
            ...
            top_reps = top_n_reps(tokenizer, data_dir, *word_sense.split('-'), method, n_reps, cluster_reps_to_present)
...
from utils.utils import timeit
@timeit
def find_top_matches(sense_positions, flat_word_clusters_dict):
    ...
    return jaccard_sims_heap

from utils.utils import timeit
@timeit
def read_clusters_for_multiple_words(tokenizer, data_dir, words, method, n_reps, cluster_reps_to_present):
    word_clusters = {}
    ...
    for word in words:
        ...
        for cluster_idx, cluster in zip(cluster_idxs, clustering_data):
            word_clusters[word][cluster_idx] = [[token, -1]] + cluster
    ...
    for word, clusters in word_clusters.items():
        for cluster_id, cluster in clusters.items():
            flat_word_clusters_dict.append(set([r for r, count in cluster][:cluster_reps_to_present]))
...
def sense_ie(app_state, tokenizer, data_dir, method, n_reps, special_tokens, num_sent_to_print):
    senses = st.text_input("Senses to search for", app_state.get('senses') or "")
